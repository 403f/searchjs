					...
					return {
						'put':function( funcName, type, content ) {
							if(typeof funcName != 'string' 
								|| message_type.indexOf(type) < 0
								|| typeof content != 'string'
							) {
								this.fire(funcName, type, content); return;
							}
						...
						'fire':function(funcName, type, content) {
							...
							( !find ? 
								console.error("the type was error where function name is"+funcName + ";type is:" + type + "log is" + content) 
								: function(){
									if(mt == 'error') {
								...
								function() {
									...
									for(let i =0 ; i < k.length; i++) {
										...
										if(data[k[i]] && typeof data[k[i]] !== 'object') {
											...
											if(typeof wh[k[i]] !== 'string'
												&& Object.getPrototypeOf(wh[k[i]]) === RegExp.prototype
											) {
